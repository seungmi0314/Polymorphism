...
public class PolyService {
	...
	public void ex1() {
		...
		//========= 업캐스팅 구간 =========
		Car car2 = new Tesla();
		//Tesla 객체를 참조하는 변수타입이 Car(부모)이기 떄문에
		...
		tesla.setFuel("전기");
		tesla.setWheel(4);
		tesla.setBetteryCapacity(100000000);
		...
		// The method setBetteryCapacity(int) is undfined for the type Car
		// => Car 타입에는 setBatteryCapacity 메소드가 정의도어있지 않다
		...
		// => Car 타입에는 setDiscountOffer 메소드가 정의도어있지 않다
		...
		Car[] arr = new Car[3]; // 부모타입 참조변수 배열 선언 및 할당
								// 각 배열의 요소가 Car타입 참조변수 여야 함
		arr[0] = car; // Car주소 == Car객체 (Car주소는 Car타입 객체)
		//arr[0] Car 참조변수
		
		arr[1] = car2;   // Tesla 주소 == Tesla 객체 / 업캐스팅 되어서 문제 안남
		//arr[1] Car 참조변수
		
		arr[2] = car3; 	// Spark주소 == Spark객체 / 얘도 업캐스팅
		// arr[2] Car 참조변수
	...
	public void ex3() {
		...
		//자식객체의 본래 필드, 메소드를 사용가능하게 하다

		Car c1 = new Tesla("전기모터", "전기", 4, 50000);
		
		//c1.tBatteryCapacity(); => c1이 부모타입
		System.out.println(((Tesla)c1).getBetteryCapacity());
		
		// 주의!
		// "."연산자가 (Tesla) 형변환 연산자보다 우선순위가 높음
		// 먼저 c1을 Tesla로 형변환 해주고, getBetteryCapacity 호출해야 함
		...
		// c1이 참조하는게 Tesla라는 걸 알기때문에
		// Tesla = Tesla 같은 자료혀 연산 가능하니 같은 주소를 복사시켜버림(얕은 복사)
		
		System.out.println(t1.getBetteryCapacity());
	...
	public void ex4() {
		...
		// 해결방법으로 instanceof와 같이 사용
		if(c1 instanceof Spark) {
			Spark s1 = (Spark)c1; // 맞을 떄 다운 캐스팅
	...
	//createCar()
	// 전달받은 매개변수에 따라서 Car 또는 자식객체를 생성하고
	// 생성된 객체의 주소를 반환
	public Car createCar(int num) {
		...
		return result;
		// 반환형이 Car이지만
		// case가 2, 3이면 Car의 자식객체의 주소가 반환된다
	}
	...
	public void ex5() {
		...
		// -> 실행 싱는 자식 (Spark)의 오버라이딩된 매소드와 바인딩 == [동적 바인딩]
		
		// ** 동적 바인딩 활용 방법 **
		Car[] arr = {
					new Car("경유 엔진", "경유", 12),
					new Tesla("전기모터", "전기", 4, 50000),
					new Spark("경차엔진", "우연", 4, 0.3)
				};
